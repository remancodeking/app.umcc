...
export default function SalaryManagementPage() {
    ...
    const handleSaveRecovery = async () => {
        ...
        const isEdit = !!newRecovery._id;
        const url = isEdit ? `/api/recovery/${newRecovery._id}` : '/api/recovery';
        ...
        try {
            ...
            if (res.ok) {
                toast.success(isEdit ? "Recovery Updated" : "Recovery Added");
        ...
        reader.onload = async (evt) => {
            try {
                ...
                for (const row of data) {
                    // Normalize Keys (iqama, lone, name)
                    const iqama = row['iqama'] || row['Iqama'] || row['IQAMA'] || row['sn'] || row['SN']; // Fallback to SN if iqama missing? No, user screenshot shows iqama column.
                    const amount = row['lone'] || row['Lone'] || row['LONE'] || row['Loan'] || row['Amount'];
                    const name = row['name'] || row['Name'];

                    /* 
                       Note: User screenshot shows 'iqama' column has the ID. 
                       We will try to match this against 'iqamaNumber' or 'empCode' in our DB.
                    */

                    if (!iqama && !name) continue; // Skip empty rows

                    // Find User
                    const user = allUsersList.find(u =>
                        (u.iqamaNumber && String(u.iqamaNumber).trim() === String(iqama).trim()) ||
                        (u.empCode && String(u.empCode).trim() === String(iqama).trim()) ||
                        (u.name && u.name.toLowerCase().trim() === String(name).toLowerCase().trim())
                    );

                    if (!user) {
                        errors.push(`Not Found: ${name} (ID: ${iqama})`);
                        continue;
                    }

                    if (amount && !isNaN(amount)) {
                        updates.push({ userId: user._id, totalAmount: Number(amount), reason: 'Loan' });
                    }
                }

                // 2. Report Errors
                if (errors.length > 0) {
                    alert(`⚠️ The following users were NOT found in the system:\n\n${errors.slice(0, 20).join('\n')}${errors.length > 20 ? '\n...and more.' : ''}`);
                    if (!confirm(`Found ${updates.length} valid records. Proceed with import?`)) return;
                } else if (updates.length > 0) {
                    if (!confirm(`Ready to import ${updates.length} recovery records?`)) return;
                } else {
                ...
                // 3. Process Updates
                const toastId = toast.loading(`Importing ${updates.length} records...`);
                let success = 0;

                // We want to UPDATE if exists, else CREATE
                // We'll process sequentially to avoid overwhelming server if many
                for (const up of updates) {
                    const existing = recoveries.find(r => r.user?._id === up.userId && r.status !== 'Completed');
                    try {
                        const url = existing ? `/api/recovery/${existing._id}` : '/api/recovery';
                        const method = existing ? 'PUT' : 'POST';
                        const body = existing ? { ...up, _id: existing._id } : up;
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center bg-purple-50 dark:bg-purple-900/10 p-6 rounded-2xl border border-purple-100 dark:border-purple-900/20">
                        ...
                        <div className="flex gap-2">
                            <label className="px-5 py-2.5 bg-green-600 text-white font-bold rounded-xl shadow-lg hover:bg-green-700 transition-colors flex items-center gap-2 cursor-pointer">
                                <Upload className="h-4 w-4" /> Import Excel
                                <input type="file" accept=".xlsx, .xls" onChange={handleFileUpload} className="hidden" />
                            </label>
                            <button onClick={() => { setNewRecovery({ userId: "", totalAmount: "", reason: "Loan" }); setIsRecoveryModalOpen(true); }} className="px-5 py-2.5 bg-purple-600 text-white font-bold rounded-xl shadow-lg hover:bg-purple-700 transition-colors flex items-center gap-2">
                                ...
                                <div className="flex justify-between items-start mb-4 mt-2">
                                    ...
                                    <div className="flex flex-col items-end gap-2">
                                        ...
                                        <div className="flex gap-1">
                                            <button onClick={() => handleEditRecovery(rec)} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors" title="Edit">
                                                <Pencil className="h-3 w-3" />
                                            </button>
                                            <button onClick={() => handleDeleteRecovery(rec._id)} className="p-1.5 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors" title="Delete">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 print:hidden">
                    <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-2xl shadow-2xl p-6">
                        <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-purple-600 dark:text-purple-400">
                            <DollarSign className="h-5 w-5" /> {newRecovery._id ? 'Edit Recovery' : 'New Recovery Entry'}
                        </h3>
                        <div className="space-y-4">
                            <div>
                            ...
                            <div className="pt-4 flex gap-2">
                                ...
                                <button onClick={handleSaveRecovery} className="flex-1 py-2.5 font-bold bg-purple-600 text-white rounded-xl shadow-lg hover:bg-purple-700">{newRecovery._id ? 'Update' : 'Add Recovery'}</button>
                            </div>
                        </div>
                    </div>
                </div>
