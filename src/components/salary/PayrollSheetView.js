...
export default function PayrollSheetView({ report, onClose }) {
  ...
  // The images show roughly 23-25 items per column.
  // We'll split the total list into two halves.
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-gray-100/90 backdrop-blur-sm flex flex-col items-center overflow-hidden">
      {/* Toolbar */}
      <div className="w-full bg-white border-b px-6 py-4 flex justify-between items-center shadow-sm print:hidden shrink-0">
        <div>
           <h2 className="text-xl font-bold">Payroll Sheet (Notebook View)</h2>
      ...
      <div className="flex-1 w-full overflow-auto p-4 md:p-8 flex justify-center bg-gray-200">
        
        {/* The Paper */}
        <div className="bg-[#fdfdfd] shadow-2xl w-full max-w-[210mm] min-h-[297mm] p-[10mm] print:shadow-none print:w-full print:max-w-none print:p-0 print:m-0 relative">
            <style jsx global>{`
                @media print {
                    @page { size: A4 portrait; margin: 0; }
                    body { background-color: white; }
                    .print\\:hidden { display: none !important; }
                    .sheet-container { margin: 0; height: 100vh; }
                }
            `}</style>
